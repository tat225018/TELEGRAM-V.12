...
def get_text_messages(bot, cur_user, message):
    ...
    elif ms_text == "Произнеси текст":
        from telebot import types
        message = bot.send_message(chat_id, text="Пришлите мне ваш текст:", reply_markup=types.ReplyKeyboardRemove())
        bot.register_next_step_handler(message, SayText_SecondPart, bot=bot)

    elif ms_text == "Главные новости":
        pass
...
# -----------------------------------------------------------------------
def SayText_SecondPart(message, bot):
    from menuBot import goto_menu

    chat_id = message.chat.id
    ms_text = message.text

    name_audio = 'Ваш текст.ogg'
    say_text(ms_text, file=name_audio)
    with open(name_audio, 'rb') as audio:
        bot.send_chat_action(message.from_user.id, 'upload_audio')
        bot.send_audio(chat_id, audio)

    goto_menu(bot, chat_id, "Голос!")
